Library of Ruina 日本語が□になるの修正ツール ver1.0.0

●説明
Library of Ruina ver 1.0.0.1iにおいて発生している、日本語表示だと文章の句読点などが「□」になる現象を修正するツールです。
(ただしステータスにカーソルを合わせるとポップする部分に書かれている文章など一部はこのツールでは修正できません)
...
で公開されているテキストにローマ数字を足したもの。
ProjectMoonがこのテキストを元にフォントを生成してくれれば□現象は解決するのでこのツールは不要になる。
(ただゲーム全部を見たわけではないのでまだこのテキストでも不足している文字はあるかも)

このツールで直らない、文章が全部□になっているのはその部分の表示で使用しているフォントが英語フォントなど日本語ではないフォントになっていると予想。
